...
ifeq ($(OS),Windows_NT)
	GLSL_COMPILER_PATH := $(realpath $(THIRD_PARTY_DIR)/shaderc/bin/win32/x64/glslc.exe)
else
	UNAME := $(strip $(shell uname -s 2>/dev/null))
	ifeq ($(UNAME),Darwin)
		GLSL_COMPILER_PATH := $(realpath $(THIRD_PARTY_DIR)/shaderc/bin/macos/x86_64/glslc)
	else
		GLSL_COMPILER_PATH := $(realpath $(THIRD_PARTY_DIR)/shaderc/bin/linux/x86_64/glslc)
	endif
endif
...
$(SPIRV_DIR)/_internal_full_screen_transfer_vertex.inl $(SPIRV_DIR)/_internal_full_screen_transfer_vertex.d : $(SHADERS_DIR)/full_screen_transfer_vertex.sl
	...
	$(HIDE) "$(GLSL_COMPILER_PATH)" $(GLSL_COMPILER_DEBUG_FLAGS) -fshader-stage=vert -std=310es -MD -MF "$(SPIRV_DIR)/_internal_full_screen_transfer_vertex.d" -mfmt=num -o "$(SPIRV_DIR)/_internal_full_screen_transfer_vertex.inl" "$(SHADERS_DIR)/full_screen_transfer_vertex.sl"
...
$(SPIRV_DIR)/_internal_full_screen_transfer_fragment.inl $(SPIRV_DIR)/_internal_full_screen_transfer_fragment.d : $(SHADERS_DIR)/full_screen_transfer_fragment.sl
	$(HIDE) $(call host-mkdir,$(SPIRV_DIR))
	$(HIDE) "$(GLSL_COMPILER_PATH)" $(GLSL_COMPILER_DEBUG_FLAGS) -fshader-stage=frag -std=310es -MD -MF "$(SPIRV_DIR)/_internal_full_screen_transfer_fragment.d" -mfmt=num  -o "$(SPIRV_DIR)/_internal_full_screen_transfer_fragment.inl" "$(SHADERS_DIR)/full_screen_transfer_fragment.sl"
