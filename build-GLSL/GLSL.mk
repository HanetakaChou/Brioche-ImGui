...
$(SPIRV_DIR)/_internal_imgui_vertex.inl $(SPIRV_DIR)/_internal_imgui_vertex.d : $(SHADERS_DIR)/imgui_vertex.bsl
	$(HIDE) $(call host-mkdir,$(SPIRV_DIR))
	$(HIDE) "$(GLSL_COMPILER_PATH)" $(GLSL_COMPILER_DEBUG_FLAGS) -fshader-stage=vert -std=310es --target-env=vulkan1.0 -MD -MF "$(SPIRV_DIR)/_internal_imgui_vertex.d" -mfmt=num -o "$(SPIRV_DIR)/_internal_imgui_vertex.inl" "$(SHADERS_DIR)/imgui_vertex.bsl"

$(SPIRV_DIR)/_internal_imgui_fragment.inl $(SPIRV_DIR)/_internal_imgui_fragment.d : $(SHADERS_DIR)/imgui_fragment.bsl
	$(HIDE) $(call host-mkdir,$(SPIRV_DIR))
	$(HIDE) "$(GLSL_COMPILER_PATH)" $(GLSL_COMPILER_DEBUG_FLAGS) -fshader-stage=frag -std=310es --target-env=vulkan1.0 -MD -MF "$(SPIRV_DIR)/_internal_imgui_fragment.d" -mfmt=num -o "$(SPIRV_DIR)/_internal_imgui_fragment.inl" "$(SHADERS_DIR)/imgui_fragment.bsl"
