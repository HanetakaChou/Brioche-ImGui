...
brx_pixel_shader_parameter_end(main)
{
    ...
    brx_float4 mesh_color_and_alpha = in_interpolated_vertex_color;

    brx_float texture_alpha = brx_sample_2d(g_font_image, g_font_sampler, texcoord).x;

    out_color = brx_float4(mesh_color_and_alpha.xyz, mesh_color_and_alpha.w * texture_alpha);
}
