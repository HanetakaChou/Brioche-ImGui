...
XCODE_BUILD_ARGUMENT_BEFORE_SCHEME=(
  "-workspace"
  "${PROJECT_DIRECTORY}/Demo-BRX-ImGui.xcworkspace"
)
...
if xcodebuild "${XCODE_BUILD_ARGUMENT_BEFORE_SCHEME[@]}" -scheme Demo-Brioche-ImGui-Build-GLSL "${XCODE_BUILD_ARGUMENT_AFTER_SCHEME[@]}"; then
    echo "xcode build Demo-Brioche-ImGui GLSL passed"
else
    echo "xcode build Demo-Brioche-ImGui GLSL failed"
    exit 1
fi

if xcodebuild "${XCODE_BUILD_ARGUMENT_BEFORE_SCHEME[@]}" -scheme Demo-BRX-ImGui "${XCODE_BUILD_ARGUMENT_AFTER_SCHEME[@]}"; then
    echo "xcode build Demo-Brioche-ImGui passed"
else
    echo "xcode build Demo-Brioche-ImGui failed"
    exit 1
fi
