...
scene_renderer::scene_renderer()
    : m_scene_color_format(BRX_PAL_COLOR_ATTACHMENT_FORMAT_A2R10G10B10_UNORM_PACK32),
...
void scene_renderer::init(brx_pal_device *device, uint32_t frame_throttling_count, ui_model_t *out_ui_model, user_camera_model_t *out_user_camera_model)
{
    // Render Pass
    {
        assert(NULL == this->m_scene_color_render_pass);
...
void scene_renderer::destroy(brx_pal_device *device)
{
    // Render Pass
    {
        assert(NULL != this->m_scene_color_render_pass);
...
void scene_renderer::on_swap_chain_attach(brx_pal_device *device, uint32_t swap_chain_image_width, uint32_t swap_chain_image_height, float intermediate_width_scale, float intermediate_height_scale)
{
    // Intermediate Images
    {
        assert(0U == this->m_intermediate_width);
        this->m_intermediate_width = swap_chain_image_width * intermediate_width_scale;

        assert(0U == this->m_intermediate_height);
        this->m_intermediate_height = swap_chain_image_height * intermediate_height_scale;
...
brx_pal_sampled_image const *scene_renderer::get_scene_color_image()
{
    return this->m_scene_color_image->get_sampled_image();
}

void scene_renderer::on_swap_chain_detach(brx_pal_device *device)
{
    // Frame Buffer
    {
        assert(NULL != this->m_scene_color_frame_buffer);
...
void scene_renderer::draw(brx_pal_graphics_command_buffer *command_buffer, uint32_t frame_throttling_index, float interval_time, ui_model_t const *ui_model, user_camera_model_t const *user_camera_model)
{
    // Scene Color Pass
    {
        command_buffer->begin_debug_utils_label("Scene Color Pass");

        float const color_clear_values[4] = {ui_model->m_clear_color[0], ui_model->m_clear_color[1], ui_model->m_clear_color[2], ui_model->m_f};
