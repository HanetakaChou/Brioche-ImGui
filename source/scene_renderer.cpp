...
void scene_renderer::init(brx_device *device, uint32_t frame_throttling_count)
{
    // Render Pass
    {
        assert(NULL == this->m_scene_color_render_pass);
...
void scene_renderer::draw(brx_graphics_command_buffer *command_buffer, uint32_t frame_throttling_index, ui_model_t const *ui_model, user_camera_model_t const *user_camera_model)
{
    // Scene Color Pass
    {
