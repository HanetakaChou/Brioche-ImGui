...
struct wsi_state_t
{
    ...
    CDXUTFirstPersonCamera m_user_camera_controller;
    user_camera_model_t m_user_camera_model;
};

static void inline ui_controller_init(ui_controller_t *ui_controller);

static void inline ui_simulate(ui_controller_t *ui_controller, ui_model_t *ui_model);

static void inline user_camera_controller_init(user_camera_model_t const *user_camera_model, CDXUTFirstPersonCamera *user_camera_controller);
...
#include <shellapi.h>
static bool Platform_OpenInShellFn_Win32Impl(ImGuiContext *, const char *path);
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    HWND window = NULL;
    {
...

        {
            ...
            window = CreateWindowExW(dw_ex_style, MAKEINTATOM(window_class), L"Brioche ImGui", dw_style, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, desktop_window, NULL, hInstance, &wsi_state);
            assert(NULL != window);
        }
        ...
        platform_io.Platform_OpenInShellFn = Platform_OpenInShellFn_Win32Impl;
    ...
    while (wsi_state.m_running)
    {
        ...
        if (wsi_state.m_running && wsi_state.m_window_width > 0 && wsi_state.m_window_height > 0)
        {
            ...
            // User Camera
            {
                ...
                DirectX::XMStoreFloat3(&eye_direction, wsi_state.m_user_camera_controller.GetEyeDir());

                DirectX::XMFLOAT3 up_direction;
                DirectX::XMStoreFloat3(&up_direction, wsi_state.m_user_camera_controller.GetUpDir());

                wsi_state.m_user_camera_model.m_eye_position[0] = eye_position.x;
                wsi_state.m_user_camera_model.m_eye_position[1] = eye_position.y;
                wsi_state.m_user_camera_model.m_eye_position[2] = eye_position.z;

                wsi_state.m_user_camera_model.m_eye_direction[0] = eye_direction.x;
                wsi_state.m_user_camera_model.m_eye_direction[1] = eye_direction.y;
                wsi_state.m_user_camera_model.m_eye_direction[2] = eye_direction.z;
...
static LRESULT CALLBACK wnd_proc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
    {
        ...
        wsi_state->m_user_camera_controller.HandleKeyDownMessage(mappedKey);
    }
        return 0;
    case WM_KEYUP:
    {
        ...
        wsi_state->m_user_camera_controller.HandleKeyUpMessage(mappedKey);
    }
        return 0;
    case WM_MOUSEMOVE:
    {
        ...
        wsi_state->m_user_camera_controller.HandleMouseMoveMessage(normalized_x, normalizedY, leftButton, middleButton, rightButton);
    }
        return 0;
    case WM_CREATE:
    {
...
static void inline ui_simulate(ui_controller_t *ui_controller, ui_model_t *ui_model)
{
    ...
    {
        ImGui::Begin("EN: Hello, world! JA: こんにちは、世界よ! ZH: 你好，世界！"); // Create a window called "Hello, world!" and append into it.
...
static void inline user_camera_controller_init(user_camera_model_t const *user_camera_model, CDXUTFirstPersonCamera *user_camera_controller)
{
    ...
    user_camera_controller->SetEyePt(eye_position);
    user_camera_controller->SetEyeDir(eye_direction);
    user_camera_controller->SetUpDir(up_direction);
}
