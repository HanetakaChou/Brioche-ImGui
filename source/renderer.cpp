...
class facade_renderer final : public renderer
{
	...
	brx_pal_graphics_command_buffer *m_graphics_command_buffers[FRAME_THROTTLING_COUNT];
	brx_pal_fence *m_fences[FRAME_THROTTLING_COUNT];

	brx_pal_sampler *m_shared_none_update_set_linear_clamp_sampler;

	brx_pal_descriptor_set_layout *m_full_screen_transfer_pipeline_none_update_descriptor_set_layout;
	brx_pal_descriptor_set *m_full_screen_transfer_pipeline_none_update_descriptor_set;
	...
	void attach_window(void *wsi_window, float intermediate_width_scale, float intermediate_height_scale) override;

	void on_window_resize(float intermediate_width_scale, float intermediate_height_scale) override;

	void detach_window() override;

	void draw(float interval_time, ui_model_t const *ui_model, user_camera_model_t const *user_camera_model) override;
...
void facade_renderer::init(void *wsi_connection, ui_model_t *out_ui_model, user_camera_model_t *out_user_camera_model)
{
	...
	// Descriptor Set Layout
	{
		...
		BRX_PAL_DESCRIPTOR_SET_LAYOUT_BINDING const full_screen_transfer_none_update_descriptor_set_layout_bindings[] = {
			{0U, BRX_PAL_DESCRIPTOR_TYPE_SAMPLER, 1U},
	...
	{
		this->m_device->write_descriptor_set(this->m_full_screen_transfer_pipeline_none_update_descriptor_set, 0U, BRX_PAL_DESCRIPTOR_TYPE_SAMPLER, 0U, 1U, NULL, NULL, NULL, NULL, NULL, NULL, &this->m_shared_none_update_set_linear_clamp_sampler, NULL);
	}

	// Pipeline Layout
	{
		assert(NULL == this->m_full_screen_transfer_pipeline_layout);
		brx_pal_descriptor_set_layout *const full_screen_transfer_descriptor_set_layouts[] = {this->m_full_screen_transfer_pipeline_none_update_descriptor_set_layout};
...
void facade_renderer::attach_window(void *wsi_window, float intermediate_width_scale, float intermediate_height_scale)
{
	assert(NULL == this->m_surface);
...
void facade_renderer::on_window_resize(float intermediate_width_scale, float intermediate_height_scale)
{
	for (uint32_t frame_throttling_index = 0U; frame_throttling_index < FRAME_THROTTLING_COUNT; ++frame_throttling_index)
	{
		this->m_device->wait_for_fence(this->m_fences[frame_throttling_index]);
...
void facade_renderer::attach_swap_chain()
{
	...
	this->m_scene_renderer.on_swap_chain_attach(this->m_device, this->m_swap_chain_image_width, this->m_swap_chain_image_height, this->m_intermediate_width_scale, this->m_intermediate_height_scale);
	...
	// Descriptor
	{
		// Full Screen Transfer Pipeline
		{
			...
			assert(NULL != this->m_full_screen_transfer_pipeline_none_update_descriptor_set_layout);
			{
				brx_pal_sampled_image const *const sampled_images[] = {
					this->m_scene_renderer.get_scene_color_image()};
				this->m_device->write_descriptor_set(this->m_full_screen_transfer_pipeline_none_update_descriptor_set, 1U, BRX_PAL_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 0U, sizeof(sampled_images) / sizeof(sampled_images[0]), NULL, NULL, NULL, NULL, sampled_images, NULL, NULL, NULL);
			}
		}
	}
}
...
void facade_renderer::create_swap_chain_compatible_render_pass_and_pipeline()
{
	...
	{
		assert(NULL == this->m_swap_chain_render_pass);
		BRX_PAL_RENDER_PASS_COLOR_ATTACHMENT color_attachments[1] = {
...
void facade_renderer::draw(float interval_time, ui_model_t const *ui_model, user_camera_model_t const *user_camera_model)
{
	...
	// Swap Chain Pass
	{
		...
		// Full Screen Transfer Pass
		{
			...
			brx_pal_descriptor_set *const descritor_sets[1] = {this->m_full_screen_transfer_pipeline_none_update_descriptor_set};
