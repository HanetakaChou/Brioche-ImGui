...
class facade_renderer : public renderer
{
	brx_device *m_device;
	...
	void draw(ui_model_t const *ui_model, user_camera_model_t const *user_camera_model) override;
...
extern renderer *renderer_init(void *wsi_connection)
{
	...
	new_unwrapped_renderer->init(wsi_connection);
...
void facade_renderer::init(void *wsi_connection)
{
	assert(NULL == this->m_device);
	this->m_device = brx_init_unknown_device(wsi_connection, false);

	// Scene Renderer Init
	this->m_scene_renderer.init(this->m_device, FRAME_THROTTLING_COUNT);
...
void facade_renderer::draw(ui_model_t const *ui_model, user_camera_model_t const *user_camera_model)
{
	...
	// Scene Color Pass
	{
		this->m_scene_renderer.draw(this->m_graphics_command_buffers[this->m_frame_throttling_index], this->m_frame_throttling_index, ui_model, user_camera_model);
	}
