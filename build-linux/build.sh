...
if make -C "${PROJECT_DIRECTORY}/../build-GLSL" -f "GLSL.mk" "${MAKE_ARGUMENT[@]}"; then
    echo "build Demo-Brioche-ImGui GLSL passed"
else
    echo "build Demo-Brioche-ImGui GLSL failed"
    exit 1
fi
...
if "${NDK_BUILD_PATH}" -C "${PROJECT_DIRECTORY}" "APP_MODULES:=Demo-BRX-ImGui" "${NDK_BUILD_ARGUMENT[@]}" ; then
    echo "ndk-build Demo-Brioche-ImGui passed"
else
    echo "ndk-build Demo-Brioche-ImGui failed"
    exit 1
fi
