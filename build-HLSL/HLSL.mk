...
all : \
	$(DXIL_DIR)/_internal_imgui_vertex.inl \
	$(DXIL_DIR)/_internal_imgui_fragment.inl

$(DXIL_DIR)/_internal_imgui_vertex.inl $(DXIL_DIR)/_internal_imgui_vertex.d : $(SHADERS_DIR)/imgui_vertex.sl
	$(HIDE) $(call host-mkdir,$(DXIL_DIR))
	$(HIDE) "$(HLSL_COMPILER_PATH)" $(HLSL_COMPILER_DEBUG_FLAGS) -T vs_6_8 -MD -MF "$(DXIL_DIR)/_internal_imgui_vertex.d" -Fo "$(DXIL_DIR)/_internal_imgui_vertex.inl" "$(SHADERS_DIR)/imgui_vertex.sl"
	$(HIDE) "$(HLSL_COMPILER_PATH)" $(HLSL_COMPILER_DEBUG_FLAGS) -T vs_6_8 -Fh "$(DXIL_DIR)/_internal_imgui_vertex.inl" -Vn "imgui_vertex_shader_module_code" "$(SHADERS_DIR)/imgui_vertex.sl"

$(DXIL_DIR)/_internal_imgui_fragment.inl $(DXIL_DIR)/_internal_imgui_fragment.d : $(SHADERS_DIR)/imgui_fragment.sl
	$(HIDE) $(call host-mkdir,$(DXIL_DIR))
	$(HIDE) "$(HLSL_COMPILER_PATH)" $(HLSL_COMPILER_DEBUG_FLAGS) -T ps_6_8 -MD -MF "$(DXIL_DIR)/_internal_imgui_fragment.d" -Fo "$(DXIL_DIR)/_internal_imgui_fragment.inl" "$(SHADERS_DIR)/imgui_fragment.sl"
	$(HIDE) "$(HLSL_COMPILER_PATH)" $(HLSL_COMPILER_DEBUG_FLAGS) -T ps_6_8 -Fh "$(DXIL_DIR)/_internal_imgui_fragment.inl" -Vn "imgui_fragment_shader_module_code" "$(SHADERS_DIR)/imgui_fragment.sl"

-include \
	$(DXIL_DIR)/_internal_imgui_vertex.d \
	$(DXIL_DIR)/_internal_imgui_fragment.d

clean:
	$(HIDE) $(call host-rm,$(DXIL_DIR)/_internal_imgui_vertex.inl)
	$(HIDE) $(call host-rm,$(DXIL_DIR)/_internal_imgui_fragment.inl)
	$(HIDE) $(call host-rm,$(DXIL_DIR)/_internal_imgui_vertex.d)
	$(HIDE) $(call host-rm,$(DXIL_DIR)/_internal_imgui_fragment.d)
