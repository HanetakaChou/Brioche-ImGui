...
$(DXIL_DIR)/_internal_imgui_vertex.inl $(DXIL_DIR)/_internal_imgui_vertex.d : $(SHADERS_DIR)/imgui_vertex.bsl
	$(HIDE) $(call host-mkdir,$(DXIL_DIR))
	$(HIDE) "$(HLSL_COMPILER_PATH)" $(HLSL_COMPILER_DEBUG_FLAGS) -T vs_6_0 -MD -MF "$(DXIL_DIR)/_internal_imgui_vertex.d" -Fo "$(DXIL_DIR)/_internal_imgui_vertex.inl" "$(SHADERS_DIR)/imgui_vertex.bsl"
	$(HIDE) "$(HLSL_COMPILER_PATH)" $(HLSL_COMPILER_DEBUG_FLAGS) -T vs_6_0 -Fh "$(DXIL_DIR)/_internal_imgui_vertex.inl" -Vn "imgui_vertex_shader_module_code" "$(SHADERS_DIR)/imgui_vertex.bsl"

$(DXIL_DIR)/_internal_imgui_fragment.inl $(DXIL_DIR)/_internal_imgui_fragment.d : $(SHADERS_DIR)/imgui_fragment.bsl
	$(HIDE) $(call host-mkdir,$(DXIL_DIR))
	$(HIDE) "$(HLSL_COMPILER_PATH)" $(HLSL_COMPILER_DEBUG_FLAGS) -T ps_6_0 -MD -MF "$(DXIL_DIR)/_internal_imgui_fragment.d" -Fo "$(DXIL_DIR)/_internal_imgui_fragment.inl" "$(SHADERS_DIR)/imgui_fragment.bsl"
	$(HIDE) "$(HLSL_COMPILER_PATH)" $(HLSL_COMPILER_DEBUG_FLAGS) -T ps_6_0 -Fh "$(DXIL_DIR)/_internal_imgui_fragment.inl" -Vn "imgui_fragment_shader_module_code" "$(SHADERS_DIR)/imgui_fragment.bsl"
