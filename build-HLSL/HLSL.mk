...
ifeq ($(OS),Windows_NT)
	HLSL_COMPILER_PATH := $(realpath $(THIRD_PARTY_DIR)/DirectXShaderCompiler/bin/win32/x64/dxc.exe)
else
	UNAME := $(strip $(shell uname -s 2>/dev/null))
	ifeq ($(UNAME),Darwin)
		HLSL_COMPILER_PATH := $(realpath $(THIRD_PARTY_DIR)/DirectXShaderCompiler/bin/macos/x86_64/dxc)
	else
		HLSL_COMPILER_PATH := $(realpath $(THIRD_PARTY_DIR)/DirectXShaderCompiler/bin/linux/x86_64/dxc)
	endif
endif
...
$(DXIL_DIR)/_internal_full_screen_transfer_vertex.inl $(DXIL_DIR)/_internal_full_screen_transfer_vertex.d : $(SHADERS_DIR)/full_screen_transfer_vertex.sl
	...
	$(HIDE) "$(HLSL_COMPILER_PATH)" $(HLSL_COMPILER_DEBUG_FLAGS) -T vs_6_0 -MD -MF "$(DXIL_DIR)/_internal_full_screen_transfer_vertex.d" -Fo "$(DXIL_DIR)/_internal_full_screen_transfer_vertex.inl" "$(SHADERS_DIR)/full_screen_transfer_vertex.sl"
	$(HIDE) "$(HLSL_COMPILER_PATH)" $(HLSL_COMPILER_DEBUG_FLAGS) -T vs_6_0 -Fh "$(DXIL_DIR)/_internal_full_screen_transfer_vertex.inl" -Vn "full_screen_transfer_vertex_shader_module_code" "$(SHADERS_DIR)/full_screen_transfer_vertex.sl"
...
$(DXIL_DIR)/_internal_full_screen_transfer_fragment.inl $(DXIL_DIR)/_internal_full_screen_transfer_fragment.d : $(SHADERS_DIR)/full_screen_transfer_fragment.sl
	$(HIDE) $(call host-mkdir,$(DXIL_DIR))
	$(HIDE) "$(HLSL_COMPILER_PATH)" $(HLSL_COMPILER_DEBUG_FLAGS) -T ps_6_0 -MD -MF "$(DXIL_DIR)/_internal_full_screen_transfer_fragment.d" -Fo "$(DXIL_DIR)/_internal_full_screen_transfer_fragment.inl" "$(SHADERS_DIR)/full_screen_transfer_fragment.sl"
	$(HIDE) "$(HLSL_COMPILER_PATH)" $(HLSL_COMPILER_DEBUG_FLAGS) -T ps_6_0 -Fh "$(DXIL_DIR)/_internal_full_screen_transfer_fragment.inl" -Vn "full_screen_transfer_fragment_shader_module_code" "$(SHADERS_DIR)/full_screen_transfer_fragment.sl"
